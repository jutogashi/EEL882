...
let text = createP("Para selecionar a função desejada você deve precionar as teclas do seu teclado:<br> 1-Desenhar poligonos; 2-Desenhar Raios;<br>3-Rotacionar Raios; 4-Transladar Raios; 5-Transladar Vertices dos Poligonos;-clique nos circulos cinzas-<br> 6-Intercecao entre Raios e Poligonos; 7- Transladar Poligonos");
text.position(10, 0);
}
...
while (w< circles.length){
   if(dist(circles[w][0], circles[w][1], mouseX, mouseY)< 15 && mouseIsPressed&&(lock==0||lock==w))  {
       circles[w][0]= mouseX;
       circles[w][1]= mouseY;
       lock=w;
...
while (m< polygons.length){
    ...
    fill(c);
    if(dist(polygons[m][n][0], polygons[m][n][1], mouseX, mouseY)< 10 && mouseIsPressed&&(lock==0||lock==m))  {
        lock=m;
